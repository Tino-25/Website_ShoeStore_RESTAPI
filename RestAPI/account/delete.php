...
if(isset($id)){
	$result = $account_obj->delete($id);
	if($result){
		$account_obj->deliver_response(200, "Đã xóa thành công", null);
	}else{
		$account_obj->deliver_response(200, "Không xóa được dữ liệu", null);
	}
}else{
	$account_obj->deliver_response(400, "Không hợp lệ - thiếu giá trị id để xóa", null);
}
