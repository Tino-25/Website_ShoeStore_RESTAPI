...
if($_SERVER['REQUEST_METHOD']=="POST"){
	...
	if(!empty($data_get)){		
		...
		$result = $banner_obj->insert($data);
		if($result){
			$banner_obj->deliver_response(200, "Thêm dữ liệu thành công", null);
		}else{
			$banner_obj->deliver_response(200, "Thêm dữ liệu thất bại", null);
		}
	}else{
		$banner_obj->deliver_response(200, "Không nhận được dữ liệu", null);
	}
}else{
	$banner_obj->deliver_response(400, "Không hợp lệ - yêu cầu phương thức POST", null);
}
