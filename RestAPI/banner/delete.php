...
if(isset($id)){
	$result = $banner_obj->delete($id);
	if($result){
		$banner_obj->deliver_response(200, "Đã xóa thành công", null);
	}else{
		$banner_obj->deliver_response(200, "Không xóa được dữ liệu", null);
	}
}else{
	$banner_obj->deliver_response(400, "Không hợp lệ - thiếu giá trị id để xóa", null);
}
