...
if(!empty($search)){
	$data = $brand_obj->search($search);
	if(empty($data))
		$brand_obj->deliver_response(200, "Dữ liệu nhận về rỗng, không tìm thấy dữ liệu nào", null);
	else
		$brand_obj->deliver_response(200, "find OK, tìm thấy dữ liệu", $data);
}else{
	$brand_obj->deliver_response(400, "Yêu cầu không hợp lệ!", null);
}
