...
if($_SERVER['REQUEST_METHOD']=="POST"){
	...
	if(!empty($data_get)){
		$color_obj->deliver_response(200, "Đã nhận được dữ liệu", null);
		$data = array();
		$data['color'] = $data_get['color'];
		$result = $color_obj->insert($data);
		if($result){
			$color_obj->deliver_response(200, "Thêm dữ liệu thành công", null);
		}else{
			$color_obj->deliver_response(200, "Thêm dữ liệu thất bại", null);
		}
	}else{
		$color_obj->deliver_response(200, "Không nhận được dữ liệu", null);
	}
}else{
	$color_obj->deliver_response(400, "Không hợp lệ - yêu cầu phương thức POST", null);
}
