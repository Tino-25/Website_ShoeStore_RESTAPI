...
if(isset($id)){
	$result = $color_obj->delete($id);
	if($result){
		$color_obj->deliver_response(200, "Đã xóa thành công", null);
	}else{
		$color_obj->deliver_response(200, "Không xóa được dữ liệu", null);
	}
}else{
	$color_obj->deliver_response(400, "Không hợp lệ - thiếu giá trị id để xóa", null);
}
