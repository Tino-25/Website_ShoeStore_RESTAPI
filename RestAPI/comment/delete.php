...
if(isset($id)){
	$result = $comment_obj->delete($id);
	if($result){
		$comment_obj->deliver_response(200, "Đã xóa thành công", null);
	}else{
		$comment_obj->deliver_response(200, "Không xóa được dữ liệu", null);
	}
}else{
	$comment_obj->deliver_response(400, "Không hợp lệ - thiếu giá trị id để xóa", null);
}
