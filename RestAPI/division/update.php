...
if($_SERVER['REQUEST_METHOD']=="POST"){
	...
	$status = $division_obj->update($data);
	if($status){
		$division_obj->deliver_response(200, "Cập nhật thành công", null);
	}else{
		$division_obj->deliver_response(200, "Cập nhật không thành công", null);
	}
}else{
	$division_obj->deliver_response(400, "Không hợp lệ - cần phương thức POST", null);
}
