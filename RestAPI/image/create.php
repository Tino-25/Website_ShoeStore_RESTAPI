...
if($_SERVER['REQUEST_METHOD']=="POST"){
	...
	if(!empty($data_get)){
		...
		$result = $image_obj->insert($data);
		if($result){
			$image_obj->deliver_response(200, "Thêm dữ liệu thành công", null);
		}else{
			$image_obj->deliver_response(200, "Thêm dữ liệu thất bại", null);
		}
	}else{
		$image_obj->deliver_response(200, "Không nhận được dữ liệu", null);
	}
}else{
	$image_obj->deliver_response(400, "Không hợp lệ - yêu cầu phương thức POST", null);
}
