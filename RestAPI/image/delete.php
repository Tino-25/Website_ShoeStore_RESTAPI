...
if(isset($id)){
	$result = $image_obj->delete($id);
	if($result){
		$image_obj->deliver_response(200, "Đã xóa thành công", null);
	}else{
		$image_obj->deliver_response(200, "Không xóa được dữ liệu", null);
	}
}else{
	$image_obj->deliver_response(400, "Không hợp lệ - thiếu giá trị id để xóa", null);
}
