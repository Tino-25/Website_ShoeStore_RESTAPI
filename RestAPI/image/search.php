...
if(!empty($search)){
	$data = $image_obj->search($search);
	if(empty($data))
		$image_obj->deliver_response(200, "Dữ liệu nhận về rỗng, không tìm thấy dữ liệu nào", null);
	else
		$image_obj->deliver_response(200, "find OK, tìm thấy dữ liệu", $data);
}else{
	$image_obj->deliver_response(400, "Yêu cầu không hợp lệ!", null);
}
