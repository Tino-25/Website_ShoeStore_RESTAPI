...
if(isset($id)){
	$result = $order_obj->delete_details($id);
	if($result){
		$order_obj->deliver_response(200, "Đã xóa thành công", null);
	}else{
		$order_obj->deliver_response(200, "Không xóa được dữ liệu", null);
	}
}else{
	$order_obj->deliver_response(400, "Không hợp lệ - thiếu giá trị id để xóa", null);
}
