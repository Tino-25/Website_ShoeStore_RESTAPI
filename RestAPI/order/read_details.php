...
if(!empty($id)){
	$data = $order_obj->read_details($id);
	if(empty($data))
		$order_obj->deliver_response(200, "Dữ liệu nhận về rỗng, không tìm thấy dữ liệu nào", null);
	else
		$order_obj->deliver_response(200, "find OK, tìm thấy dữ liệu", $data);
}else{
	$order_obj->deliver_response(400, "Yêu cầu không hợp lệ!", null);
}
